...
yesweb.f=
{
	...
	uuid:function(sStart)
	{
		...
		while(s=="")
			...
			if(yesweb.f.temp.uuid[s])
				{
				s='';
				}
			else
				{
				yesweb.f.temp.uuid[s]=s;
				}
	 ...
	 * yesweb.f.delay(function() {return typeof jQuery === "function";}, function() {alert('jQuery has loading');}, 500);
	 */
	delay:function(f1,f2,iTimer)
	{
		if(f1())
			...
			{
				setTimeout(function(){ yesweb.f.delay(f1,f2,iTimer);},iTimer?iTimer:100);
			}
	},
	//延迟加载  该加载会判断juery是否加载完成
	delay_ready:function(fCall)
	{
		yesweb.f.delay(function() {
			return typeof jQuery === "function";
		}, function() {
			yesweb.f.ready(function() {
				fCall();
			});
		}, 500);
	}
